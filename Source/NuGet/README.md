...
| 25.3107.4042 | Updated **HtmlAttributeCollection.GetAttributes(node, attributeNames)** to return case-insensitive matches for all entries on the caller's array; Updated **HtmlAttributeCollection.GetAttributeValue(node, attributeName)** to return a case-insensitive match; Updated **HtmlAttributeCollection.GetValue(attributeName)** to return a case-insensitive match; Updated **HtmlAttributeCollection.HasAttribute(attributeName)** to return an indication of whether the specified match exists in any case; Updated **HtmlAttributeCollection.Remove(attributeName)** to remove all attributes from the collection whose name matches the attributeName argument in any case; Updated **HtmlAttributeCollection.SetAttribute(attributeName, value)** to find and update the first attribute whose name matches in any case; Updated **HtmlAttributeCollection.SetAttributeValue(node, attributeName, attributeValue)** to find and update the first attribute whose name matches in any case; Updated **HtmlNodeItem.FindAttribute(baseNode, expression, attributeNames)** to match attribute names in any case; Updated **HtmlNodeItem.FindAttribute(baseNode, expression, attributeNames, foundList)** to match attribute names in any case; Updated **HtmlNodeItem.GetId(node)** to return a reference to the first attribute with the case-insensitive name of 'id'; Updated **HtmlNodeItem.Id** property to set the value of an existing 'id' attribute whose name is spelled in any case; Updated the **HtmlNodeItem.Name** property set the value of an existing 'name' attribute whose name is spelled in any case; Updated **HtmlNodeItem.RemoveAttribute(node, attributeName)** to remove attributes whose names match in any case. |
| 25.3013.4544 | Added static function **HtmlUtil.EstimateFontHeight(fontSize)** to return the estimated height of the font, in pixels; Added static function **HtmlUtil.EstimateTextHeight(text, fontSize)** to return the estimated height of the text, in pixels; Added the static function **HtmlUtil.EstimateTextWidth(text, fontSize)** to return the estimated width of the text; Added the static function **HtmlAttributeCollection.GetActiveStyle(node, styleName, defaultValue)**, which returns the nearest instance of the specified style starting at the provided node and working back up the list of ancestors; Added the static function **HtmlAttributeCollection.HasAncestorStyle(node, styleName)**, which discovers whether the specified style is defined within the node's parent or ancestor chain. |
| 25.2910.4125 | Corrected an issue where, when LineFeedSeparation == true and PreserveSpace == false, and a node's closing tag was directly trailed by non-whitespace text, a new line was being introduced in the rendered HTML prior to the trailing text. |
| 25.2904.4408 | When the PreserveSpace flag is false while parsing an element with one or more properties and multiple whitespace characters between properties, the state machine now waits in name mode until a legal character is found, instead of indexing to value mode. |
